...
; 		jsr	PlaySound_Special			; play ID
		...
		lea	(v_objspace).w,a1			; load object ram address to a1
...
SRG_ClearObjects:
		...
		move.l	#$FFF8FFD4,(v_scrposy_dup).w		; set Y scroll positions
...
SRG_DumpPal:
		...
		jsr	PaletteFadeIn				; fade palettes in
		moveq	#$00,d0					; clear d0
...
SSRGScreen_Loop:
		move.b	#$04,(v_vbla_routine).w			; set V-Blank routine to run
		jsr	WaitForVBla				; hult til V-Blank begins
...
SRG_ScrollFG:
		lea	(v_scrolltable+$100).w,a1		; load the horizontal scroll buffer address
		moveq	#$00,d0					; clear d0
		move.w	($FFFF7800).l,d0			; load timer as position
		addi.w	#$0030,d0				; increase to scroll in sooner
		cmpi.w	#$00FC,d0				; has timer reached finish?
...
SRG_FinFGScroll:
		lea	(v_scrolltable).w,a1			; load the horizontal scroll buffer address
