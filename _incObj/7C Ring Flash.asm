...
Flash_Collect:
		...
		addq.b	#1,obFrame(a0)
		cmpi.b	#$20,obFrame(a0); has animation	finished?
		bcc.s	Flash_End	; if yes, branch
		cmpi.b	#3,obFrame(a0)	; is 3rd frame displayed?
		bne.s	locret_9F76	; if not, branch
                addi.b  #$80,(v_lastlamp).w ; remember that we're coming from a special stage but don't clear the lamppost data
                jsr     Lamp_StoreInfo2 ; save data
                bsr.w   SaveState       ; save level item state
