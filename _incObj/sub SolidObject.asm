...
SolidObject:
	...
	@stand:
		move.w	d4,d2
		bsr.w	MvSonicOnPtfmJump
		moveq	#0,d4
		rts
; ===========================================================================
SolidObjectSlope:
		tst.b	obSolid(a0)	; is Sonic standing on the object?
		beq.w	SolidObject2F	; if not, branch
		move.w	d1,d2
		add.w	d2,d2
		lea		(v_player).w,a1
		btst	#1,obStatus(a1)	; is Sonic in the air?
...
Solid_SideAir:
                cmpi.b  #id_springs,(a0)       ; don't walljump on springs
                beq.s   Solid_SideAir2
	;Mercury Wall Jump
		move.l	a0,-(sp)
		movea.l	a1,a0
		jsr	WallJump
...
Solid_Ignore:
		...
		cmp.b	#id_Drown,obAnim(a1); check if in drowning animation
		beq.s	Solid_NotPushing
		cmp.b	#id_Hurt,obAnim(a1); check if in hurt animation
		beq.s	Solid_NotPushing
