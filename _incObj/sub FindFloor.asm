...
; ||||||||||||||| S U B	R O U T	I N E |||||||||||||||||||||||||||||||||||||||
ColisionChkLayer:
		tst.b	(v_layer).w				; MJ: is collision set to first?
...
FindFloor:				; XREF: Sonic_AnglePos; et al
		bsr.w	FindNearestTile ; was bsr.s	FindNearestTile
...
@issolid:
	...
	;	lea	(CollArray1).l,a2
		...
		beq.s	@noflip3 ;loc_14A3E				; MJ: if not, branch
		neg.w	d0					; MJ: reverse solid value

	@noflip3:
		tst.w	d0					; MJ: is the solid data null?
		beq.s	@isblank ;loc_149DE				; MJ: if so, branch
		bmi.s	@negfloor ;loc_14A5A				; MJ: if it's negative, branch
		cmpi.b	#$10,d0					; MJ: is it 10?
		beq.s	@maxfloor ;loc_14A66				; MJ: if so, branch
