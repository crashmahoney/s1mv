...
@move:
		...
		move.b	#$10,$2A(a0)
		move.w	#-$C0,obVelX(a0)
		move.w	#$40,obAnim(a0)  ; was obInertia(a0), changed cos it broke when rearranging object variables 
		bchg	#4,$2B(a0)
		bne.s	loc_16AFC
		clr.w	obVelX(a0)
		neg.w	obAnim(a0)       ; another obInertia(a0)
...
@loc_16B5E:
		...
		move.w	#0,obVelX(a0)
			...
			clr.w	obVelX(a0)
			clr.w	obAnim(a0)      ; another obInertia(a0)
		endc
		rts	
; ===========================================================================
...
Cat_BodySeg1:	; Routine 4, 8
		...
		beq.w	loc_16C64
		move.w	obAnim(a1),obAnim(a0)       ; obInertia(a1),obInertia(a0)
		move.w	obVelX(a1),d0
		if Revision=0
		add.w	obAnim(a1),d0               ; obInertia(a1),d0
		else
			add.w	obAnim(a0),d0       ; obInertia(a0),d0
		endc
		move.w	d0,obVelX(a0)
		move.l	obX(a0),d2
		move.l	d2,d3
...
loc_16CAA:
		...
		move.b	#$98,obColType(a0)   ; bug fix, Stops sonic getting hurt when destroying Caterkiller at speed
